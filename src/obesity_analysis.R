...
# LES COURBES
plot(1 - roc_train$specificities, roc_train$sensitivities, 
      col = "blue", lwd = 2, type = "l")
points(1 - roc_train$specificities, roc_train$sensitivities, 
     col = "blue", lwd = 2)
...
###########################
# PREPARATION POUR XGBOOST
###########################

# Liste des variables explicatives
vars <- c('RIAGENDR', 'RIDAGEYR', 'DR1IALCO', 'DR1IPROT', 'DR1ISUGR', 
          'DR1ICHOL', 'DR1IVB12', 'DR1IVD', 'DR1ICALC', 'DR1ISODI', 
          'DR1IPOTA', 'DBD900', 'MCQ366B')

# One-Hot Encoding pour XGBoost
train_mat <- model.matrix(obesite ~ . , data = train_data[, c(vars, "obesite")])[,-1]
validation_mat <- model.matrix(obesite ~ . , data = validation_data[, c(vars, "obesite")])[,-1]
test_mat <- model.matrix(obesite ~ . , data = test_data[, c(vars, "obesite")])[,-1]

# Cibles binaires 0/1
train_label <- as.numeric(train_data$obesite) - 1
validation_label <- as.numeric(validation_data$obesite) - 1
test_label <- as.numeric(test_data$obesite) - 1

# Création des DMatrix
dtrain <- xgb.DMatrix(data = train_mat, label = train_label)
dval   <- xgb.DMatrix(data = validation_mat, label = validation_label)
...
params <- list(
  ...
  min_child_weight = 1,
  scale_pos_weight = 2.171472  # équilibrage
)
...
pred_class_val <- ifelse(pred_val > 0.5, 1, 0)

# Accuracy
accuracy_val <- mean(pred_class_val == validation_label)
accuracy_val

# Confusion matrix
table(Predit = pred_class_val, Reel = validation_label)

###########################
# ROC + AUC
###########################

roc_val <- roc(validation_label, pred_val)
auc(roc_val)
plot(roc_val, main = "ROC XGBoost - Validation")
...
# LES COURBES
plot(1 - roc_val$specificities, roc_val$sensitivities, 
     col = "blue", lwd = 2, type = "l")
points(1 - roc_val$specificities, roc_val$sensitivities, 
       col = "blue", lwd = 2)

# AJOUT DES AUC
text(x = 0.6, y = 0.4, 
     labels = paste("Train AUC =", round(auc(roc_val), 3)),
     col = "blue", adj = 0)
...
# Préparer les données pour PRC : labels 0/1 et probabilités
y_true_val <- validation_label               
prob_val <- pred_val                          

# Calcul de la courbe PRC
prc_val <- pr.curve(scores.class0 = prob_val, 
                    weights.class0 = y_true_val, 
                    curve = TRUE)

# Affichage des résultats
cat("AUC-PR Validation :", round(prc_val$auc.integral, 3), "\n")

# Tracer la courbe PRC
plot(prc_val, 
     color = "red", 
     lwd = 2,
     main = "PRC XGBoost - Validation",
     xlab = "Recall (Sensibilité)",
     ylab = "Precision")

# Ajouter ligne de référence : précision aléatoire
baseline_precision <- mean(y_true_val)
abline(h = baseline_precision, lty = 2, col = "gray")


weight_final = ifelse(train_data$obesite == "Obese",2.17,1)


# 1. ENTRAÎNEMENT DE LA FORÊT
set.seed(2025)
foret <- randomForest(
  obesite ~ RIAGENDR + RIDAGEYR + DR1IALCO + 
    DR1IPROT + DR1ISUGR + DR1ICHOL + DR1IVB12 + DR1IVD + 
    DR1ICALC + DR1ISODI + DR1IPOTA + DBD900 + MCQ366B,
  data = train_data,
  ntree_val = 800,           # Nombre d'arbres
  mtry_val = 9,       # Variables testées à chaque split (√p)
...
importance_foret <- importance(foret)
print(importance_foret)

importance_df <- as.data.frame(importance(foret))
...
# 3. PRÉDICTIONS ET PERFORMANCES
cat("\n=== PERFORMANCES FORÊT ALÉATOIRE ===\n")
...
# Calcul Sensibilité et Spécificité pour l'arbre
tp_arbre <- sum(pred_validation == "Obese" & validation_data$obesite == "Obese")
fp_arbre <- sum(pred_validation == "Obese" & validation_data$obesite == "Non_obese")
fn_arbre <- sum(pred_validation == "Non_obese" & validation_data$obesite == "Obese")
tn_arbre <- sum(pred_validation == "Non_obese" & validation_data$obesite == "Non_obese")

sens_arbre <- tp_arbre / (tp_arbre + fn_arbre)
spec_arbre <- tn_arbre / (tn_arbre + fp_arbre)

# Calcul Sensibilité et Spécificité pour la forêt
tp_foret <- sum(pred_foret_validation == "Obese" & validation_data$obesite == "Obese")
fp_foret <- sum(pred_foret_validation == "Obese" & validation_data$obesite == "Non_obese")
fn_foret <- sum(pred_foret_validation == "Non_obese" & validation_data$obesite == "Obese")
tn_foret <- sum(pred_foret_validation == "Non_obese" & validation_data$obesite == "Non_obese")

sens_foret <- tp_foret / (tp_foret + fn_foret)
spec_foret <- tn_foret / (tn_foret + fp_foret)

# Calcul Sensibilité et Spécificité pour XGBoost
pred_class_xgb_val <- ifelse(pred_val > 0.5, "Obese", "Non_obese")
pred_class_xgb_val <- factor(pred_class_xgb_val, levels = c("Non_obese", "Obese"))

tp_xgb <- sum(pred_class_xgb_val == "Obese" & validation_data$obesite == "Obese")
fp_xgb <- sum(pred_class_xgb_val == "Obese" & validation_data$obesite == "Non_obese")
fn_xgb <- sum(pred_class_xgb_val == "Non_obese" & validation_data$obesite == "Obese")
tn_xgb <- sum(pred_class_xgb_val == "Non_obese" & validation_data$obesite == "Non_obese")

sens_xgb <- tp_xgb / (tp_xgb + fn_xgb)
spec_xgb <- tn_xgb / (tn_xgb + fp_xgb)

# Synthèse comparative complète
cat("\n=== SYNTHÈSE COMPARATIVE ===\n")
cat("                | ARBRE  | FORÊT  | XGBOOST | GAIN (XGB - ARBRE)\n")
cat("Accuracy Val    |", round(mean(pred_validation == validation_data$obesite), 4), "|", 
    ...
    round(accuracy_val - mean(pred_validation == validation_data$obesite), 4), "\n")
cat("Sensibilité Val |", round(sens_arbre, 4), "|", 
    round(sens_foret, 4), "|", 
    round(sens_xgb, 4), "|", 
    round(sens_xgb - sens_arbre, 4), "\n")
cat("Spécificité Val |", round(spec_arbre, 4), "|", 
    round(spec_foret, 4), "|", 
    round(spec_xgb, 4), "|", 
    round(spec_xgb - spec_arbre, 4), "\n")
